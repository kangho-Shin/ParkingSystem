...
public sealed class LprLaneProcessorTests
{
    ...
    [Fact]
    public async Task 파일의_장치번호를_실제DeviceId로_변환한다()
    {
        Guid eventId = Guid.NewGuid();
        FieldEventResponse response = new(eventId, true, 77, "OK", "입차", true);
        RecordingDisplayOutput output = new();
        await using TestContext context = await TestContext.CreateAsync(
            "Entry", HttpStatusCode.OK, JsonConvert.SerializeObject(response), output,
            siteId: 9001, laneId: 9010, deviceId: 4001, deviceNumber: 401);
        string fileName = $"9001_002_401_9010_Entry_20260920103000123_12가3456_{eventId:N}.jpg";

        LprProcessResult result = await context.Processor.ProcessAsync(
            fileName, CancellationToken.None);

        Assert.True(result.Acknowledged);
        Assert.Equal(4001, output.Recognition!.DeviceId);
        Assert.Equal(9010, output.Recognition.LaneId);
    }
    ...
    private sealed class TestContext : IAsyncDisposable
    {
            ...
            int deviceNumber = 101)
        {
            ...
            LocalBootstrapStore bootstrap = new(connectionString);
            ...
            await store.SaveAsync(new SiteConfiguration(
                ...
                new[] { new ParkingLane(laneId, siteId, 2, "시험차로", laneDirection, true) },
                new[] { new ParkingDevice(deviceId, siteId, laneId, deviceNumber, "LPR", "시험LPR", null, true) }),
            ...
            LprLaneProcessor processor = new(
                ...
                new LocalConfigurationService(bootstrap, store),
                eventService,
