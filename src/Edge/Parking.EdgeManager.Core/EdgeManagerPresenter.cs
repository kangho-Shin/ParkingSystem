...
public sealed class EdgeManagerPresenter : IDisposable
{
    ...
    private async Task ShowEntryAsync(
        ...
        CancellationToken cancellationToken)
    {
        byte[]? inImage = await _client.GetImageAsync(entry.InImage, cancellationToken);
        _view.ShowImages(inImage, null);
    }
    ...
    private async Task ShowActivityAsync(
        ...
        CancellationToken cancellationToken)
    {
        byte[]? inImage = await _client.GetImageAsync(activity.InImage, cancellationToken);
        byte[]? outImage = await _client.GetImageAsync(activity.OutImage, cancellationToken);
        _view.ShowImages(inImage, outImage);
    }
