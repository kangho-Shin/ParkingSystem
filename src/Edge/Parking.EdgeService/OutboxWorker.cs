        ...
        public async Task ProcessOnceAsync(CancellationToken cancellationToken)
        {
            ...
            foreach (OutboxMessage message in messages)
            {
                try
                {
                    ...
                    if (message.EventType == "KioskOfflineExit")
                    {
                        OfflineKioskExitRequest request =
                            JsonConvert.DeserializeObject<OfflineKioskExitRequest>(message.PayloadJson)
                            ?? throw new InvalidOperationException("무인 장애출차 Outbox 데이터를 읽지 못했습니다.");
                        response = await _gatewayClient.SendOfflineKioskExitAsync(request, cancellationToken);
                        if (response.EventId != message.EventId)
                            throw new InvalidOperationException("응답 EventId가 다릅니다.");
                        if (_monitoring is not null)
                        {
                            ExitEventRequest exit = new(
                                request.EventId, request.SiteId, request.LaneId, request.DeviceId,
                                request.CarNumber, request.OutDateTime, request.Groupnum,
                                EventType: ParkingEventType.Exit, OutImage: request.OutImage);
                            await _monitoring.RecordExitAsync(
                                exit,
                                response with { OpenBarrier = true },
                                EdgeDeliveryState.Completed,
                                cancellationToken);
                        }
                    }
