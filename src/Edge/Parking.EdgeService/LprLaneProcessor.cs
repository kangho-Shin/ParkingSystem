...
public sealed class LprLaneProcessor
{
    ...
    private readonly KioskExitCoordinator? _kioskExitCoordinator;
    ...
    public LprLaneProcessor(
        ...
        KioskExitCoordinator? kioskExitCoordinator = null)
    {
        _parser = parser;
        ...
        _kioskExitCoordinator = kioskExitCoordinator;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        long siteId;
        try { siteId = await _configurationService.GetSiteIdAsync(cancellationToken); }
        catch (InvalidOperationException) { return Reject(recognition.EventId, "CONFIG_NOT_READY"); }
        if (recognition.SiteId != siteId)
            return Reject(recognition.EventId, "INVALID_SITE");

        SiteConfiguration? configuration =
            await _configurationService.GetAsync(cancellationToken);
        ...
        ParkingDevice? device = configuration.Devices.FirstOrDefault(
            value => value.DeviceNumber == recognition.DeviceId);
        ...
        recognition = recognition with { DeviceId = device.DeviceId };
        ...
        try
        {
            ...
            {
                response = _kioskExitCoordinator is not null
                    ? await _kioskExitCoordinator.RouteAsync(recognition, cancellationToken)
                    : await _eventService.AcceptExitAsync(
