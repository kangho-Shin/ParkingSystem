...
namespace Parking.EdgeGateway
{
    ...
    public sealed class EdgeGatewayController : ControllerBase
    {
        ...
        [HttpPost("exits/kiosk-offline-open")]
        public Task<IActionResult> OfflineKioskExitAsync(
            [FromBody] OfflineKioskExitRequest request, CancellationToken cancellationToken) =>
            RelayAsync(() => _relay.RelayOfflineKioskExitAsync(request, cancellationToken));
        ...
        [HttpGet("config/sites/{siteId:long}/versioned")]
        public async Task<IActionResult> GetVersionedConfigurationAsync(
            long siteId,
        ...
        [HttpPut("config/sites/{siteId:long}/versioned")]
        public Task<IActionResult> SaveVersionedConfigurationAsync(
            long siteId,
            [FromBody] JToken request,
            CancellationToken cancellationToken) => RelayHttpAsync(() =>
                _client.SaveVersionedConfigurationAsync(
                    siteId,
                    Request.Headers["X-Site-Key"].ToString(),
        ...
        [HttpPut("parking/sessions/{parkingSessionId:long}/car-number")]
        public Task<IActionResult> CorrectCarNumberAsync(long parkingSessionId, [FromBody] JToken request, CancellationToken cancellationToken) =>
            RelayHttpAsync(() => _client.RelayCarNumberCorrectionAsync(
                parkingSessionId, request.ToString(Newtonsoft.Json.Formatting.None), cancellationToken));
