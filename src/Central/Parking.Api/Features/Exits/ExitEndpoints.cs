...
namespace Parking.Api.Features.Exits
{
    ...
    public sealed class ExitController : ControllerBase
    {
        ...
        public async Task<IActionResult> OfflineKioskOpenAsync(
            [FromBody] OfflineKioskExitRequest request,
            CancellationToken cancellationToken)
        {
            if (request.EventId == Guid.Empty || request.SiteId <= 0 ||
                request.Groupnum <= 0 || request.LaneId <= 0 || request.DeviceId <= 0 ||
                string.IsNullOrWhiteSpace(request.CarNumber) || request.OutDateTime == default)
                return BadRequest();
            ...
            return Ok(await _repository.SaveExitAsync(
                exit, true, cancellationToken, "KIOSK_OFFLINE_OPEN"));
        }
    }
}
