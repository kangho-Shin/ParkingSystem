...
파일명은 `SSS_GGG_DDD_LLL_Direction_yyyyMMddHHmmssfff_CarNumber_EventId.jpg`이며 EventId는 Guid `N` 형식 32자리다. `SSS`와 `LLL`은 실제 `sitenum`, `laneid` 숫자이고, `DDD`는 카메라의 3자리 `devicenum`이다. EdgeService는 `devicenum`을 실제 `deviceid`로 변환한다.
...
| PUT | `/api/v1/config/device-links` | `ParkingDeviceLink` | 204 |
...
| POST | `/api/v1/edge/exits/kiosk-offline-open` | API `/api/v1/parking/exits/kiosk-offline-open` |
...
| POST | `/api/v1/local/kiosks/events/{eventId}/complete` | 본문의 `Sitenum/Groupnum/Devicenum`을 내부 DeviceId로 변환한 뒤 무인 출차 완료 및 전광판·차단기 제어 |

SignalR Hub는 `/hubs/kiosk`이며 연결 후 `Register(sitenum, groupnum, devicenum)`을 호출한다. EdgeService가 내부 DeviceId로 변환하며 출차 알림 메서드는 `ExitVehicleDetected`이다.
...
## 6. 차량번호 검색과 등록차량 후속 연동

일반차량 검색과 등록차량 입·출차 및 회원 관리 API는 구현됐다. 아래 검색 API에 등록차량 후보를 함께 반환하는 통합은 아직 구현하지 않았다.

### GET `/api/v1/parking/search?siteId={siteId}&groupnum={groupnum}&carNumber={number}`

- `carNumber`에 전체 차량번호 또는 뒤 4자리를 입력
- 현재 일반차량 미출차 내역 반환
- 향후 등록차량 후보 통합 여부는 무인정산 화면 연동 시 확정
- 최신 입차시간순 정렬
...
- `parking_device_link`에 LPR·무인정산기·전광판 장비 관계를 등록하고, 전광판의 `IpAddress`, TCP `Port`를 저장한다.
- EdgeService는 설정을 동기화한 뒤 LPR 처리 결과를 해당 차로 전광판으로 전송한다.
- `OpenBarrier=true`일 때만 전광판에 차단기 열림 명령을 전송한다.
- 한 장비에 여러 연결을 둘 수 있으며 LPR과 무인정산기는 각각 전광판에 연결할 수 있다.
