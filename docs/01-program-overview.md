...
| `Parking.KioskSimulator` | 개발 PC | SignalR 출차 알림 수신과 무인 완료 회신 시험 | 구현 완료 |
...
| `Parking.TerminalAgent` | 현장 Windows 서비스 | 백그라운드 프로그램 자동실행, 비정상 종료 재시작, 반복 장애 제한, 파일 로그 | 구현 완료·Windows 실행시험 대기 |

현장 실행 프로그램은 공통으로 `sitenum`, `groupnum`, `devicenum`을 사용한다. `Parking.EdgeService`가 현장 설정에서 전역 고유 내부키 `deviceid`를 찾아 장비 연결과 중앙 API 요청에 사용한다.
...
- `outbox_message`: 입차·출차·결제·무인 장애출차 중앙 전송 대기열
...
출구 LPR과 무인정산기는 `parking_device_link`로 연결한다. 연결된 무인이 온라인이면 SignalR로 출차 사건을 전달하고, 무인이 완료 회신한 뒤 전광판과 차단기를 제어한다. 무인이 오프라인이면 그룹별 `CMD_KIOSK_OFFLINE_POLICY`의 `OPEN` 또는 `BLOCK` 정책을 적용한다. `OPEN` 장애출차는 먼저 SQLite Outbox에 저장되므로 중앙 장애 중에도 복구 후 자동 전송된다.
