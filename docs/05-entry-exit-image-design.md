# 입출차 및 차량 이미지 설계

현재 문서는 입출차 및 이미지 구현의 확정 설계다.

## 1. 공통 원칙

모든 입차·출차 사건에는 다음 정보가 있어야 한다.

- `EventId`
- `EventType`: `Entry` 또는 `Exit`
- `Sitenum`, `Groupnum`, `LaneId`, `DeviceId`
- 차량번호
- 입차 `InDateTime` 또는 출차 `OutDateTime`
- 입차 `InImage` 또는 출차 `OutImage` 파일명

요청의 `EventType`과 `parking_lane.Direction`이 다르면 요청을 거부한다.

## 2. 일반차량

일반차량은 별도의 입차·출차 테이블로 이동하지 않고 `parking_session` 한 행을 계속 사용한다.

### 입차

- `parking_event`에 Entry 사건 저장
- `parking_session` 신규 행 생성
- 차량번호, 입차시각, 입차 차로·장비, 입차 이미지 저장
- `outflag`를 `I`로 설정

### 정산

- 같은 `parking_session`에 결제 내역 연결
- 전체 차량번호와 뒤 4자리 조회 모두 같은 요금계산 흐름을 사용
- 차량조회나 요금계산만으로는 `outflag=I`를 유지
- 실제 결제완료 또는 최종요금 0원 확정 시 `outflag=X`로 변경

### 출차

- 최신 미출차 `parking_session` 조회
- 출차시각, 출차 차로·장비, 출차 이미지 저장
- `outflag`를 `O`로 변경
- 입차정보는 삭제하지 않는다.

`parking_session`에는 기존 시스템과 같은 `outflag CHAR(1)`를 사용한다.

```text
I = 입차
X = 실제 정산완료
O = 출차완료

I → X → O
```

현재의 문자열 `status`와 `outflag`를 함께 사용하지 않고 `outflag`로 통일한다. 결제 상세는 `payment`와 `paydate`에서 확인한다.

## 3. 등록차량

- 등록 원본은 `tperiodmember`에서 조회한다.
- 입차 시 등록차량 여부와 회원 식별정보를 확정한다.
- 입차 내역은 `tperiodinout`에 신규 생성한다.
- 출차 시 해당 미출차 행을 찾아 출차정보를 업데이트한다.
- 차량번호, 입·출차시각, 입·출차 차로·장비, 입·출차 이미지를 보존한다.
- 주차 중 등록기간이나 등록정보가 변경되어도 입차 당시 판정값을 유지한다.

## 4. 차량번호 뒤 4자리 조회

출구무인과 사전무인은 전체 차량번호 또는 차량번호 뒤 4자리로 미출차 차량을 조회한다.

- `Sitenum`, `Groupnum` 범위 적용
- 일반차량과 등록차량 미출차 내역을 함께 조회
- 최신 입차시간순 정렬
- 차량번호 전체값, 입차시간, 입차이미지, 차량구분 반환
- 전체 차량번호 조회 결과가 한 건이면 즉시 요금 계산
- 뒤 4자리 조회 결과가 한 건이면 즉시 요금 계산
- 결과가 여러 건이면 정산기에 차량목록을 보내 사용자가 선택
- 선택된 `ParkingSessionId`로 다시 호출하여 요금 계산
- 조회·계산만으로는 `outflag=I`를 유지
- 결제완료 또는 최종요금 0원 확정 시 `outflag=X`로 변경

뒤 4자리 검색 결과를 임의로 한 건만 선택하지 않는다.

## 5. 출구 LPR

출구 LPR 인식 결과에는 다음 값을 포함한다.

- 차량번호
- 출차시각
- 출차 이미지
- 출차 차로·장비
- `EventId`, `EventType=Exit`

차량번호로 미출차 내역을 찾고 현재 요금과 결제상태를 확인한 뒤 차단기 개방 여부를 결정한다.

## 6. 이미지 파일명

```text
SSS_GGG_DDD_LLL_Entry_yyyyMMddHHmmssfff_차량번호_EventId.jpg
SSS_GGG_DDD_LLL_Exit_yyyyMMddHHmmssfff_차량번호_EventId.jpg
```

예:

```text
9001_002_101_9010_Entry_20260919153025123_12가3456_4a912811cb4d4c3fa934de51805a52d1.jpg
```

- 차량번호에서 파일명에 사용할 수 없는 문자를 제거한다.
- `Sitenum`과 `LaneId`는 실제 DB 식별자를 그대로 기록한다.
- `Groupnum`과 카메라 `Devicenum`은 3자리로 기록한다.
- EdgeService는 `Devicenum`을 설정의 실제 `DeviceId`로 변환한다.
- 밀리초와 EventId를 포함하여 파일명 충돌을 막는다.
- DB에는 이미지 바이너리나 전체 경로가 아니라 파일명만 저장한다.

## 7. 이미지 저장과 전송

- 이미지 파일 전송과 웹 조회는 기존 구현을 재사용한다.
- 이번 프로젝트는 이미지 파일을 전송하지 않고 `InImage`, `OutImage` 파일명만 DB에 저장한다.

## 8. 중복 처리

- 같은 `EventId`는 최초 처리 결과를 반환한다.
- `CMD_DUPLICATE_ENTRY_TIME`은 사이트·그룹별 중복입차 판단시간이며 기본값은 10초다.
- 같은 사이트·그룹·차량번호의 `I`가 10초 이내 다시 들어오면 기존 `ParkingSessionId`를 반환한다.
- 10초가 지난 이전 `I`는 이전 세션과 입차 이벤트를 삭제한 뒤 새로 입차한다.
- 이전 상태가 `X`이면 결제자료를 보존하기 위해 `O`로 변경한 뒤 새 `I`를 생성한다.
- 중복 검사 기준은 사이트, 차로 방향, 차량번호, `InDateTime` 또는 `OutDateTime`을 사용한다.
- 보조 LPR 결과 결합 규칙은 LprHost 구현 단계에서 별도로 확정한다.

## 9. 다음 구현 순서

1. 공통 계약에 `EventType`, `Groupnum`, 이미지 정보를 추가
2. MySQL 주차 세션과 등록차량 입출차 스키마에 이미지·차로·장비 필드 추가
3. SQLite 이미지 전송대기 구조 추가
4. 전체 차량번호·뒤 4자리 통합조회 API 구현
5. 입차 API와 출차 API에 방향 검증 적용
6. Simulator에 이미지·출차·4자리 조회 시험 추가
