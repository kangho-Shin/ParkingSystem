...
namespace APSMain
{
    public partial class ParkCalForm : Form, ISubFormResult<string>, IActiveForm
    {
        ...
        public CardTransInfo? _currentTransInfo;
        ...
        public int _carType = 1; // 1: 소형  2: 중형 3: 대형
        ...
        public ParkCalForm(int ldmIndex, bool periodtype)
        {
            InitializeComponent();
            ...
            _mainForm = APSConfig.mainForm
                        ?? throw new InvalidOperationException("MainForm이 초기화되기 전에 ParkCalForm이 생성되었습니다.");
            if (_mainForm != null) {
        ...
        private async void ParkCalForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            try {
                ...
                XLogClass?.SaveLogString("CAL", $"ParkCalForm Closing Error: {ex}");
            }
            finally {
                ...
                XLogClass?.SaveLogString("CAL", "ParkCalForm Closing OK");
                XLogClass?.SaveLogString("CAL", "==============================================================");
            }
        }

        private void ParkCalForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            CloseTicketReader();
        }
        ...
        private void CloseTicketReader()
        {
            ...
            try {
                ...
                Console.WriteLine("TicketReader Close OK");
        ...
        private async void ParkCalFrm_Load(object sender, EventArgs e)
        {
            int disPersonKey = 0;
            ...
            if (_parkinfo != null) {
                ...
                if (_carType < 1 || _carType > 5) { _carType = 1; }
            ...
            if (_parkRemainFee > 0) {
                ...
                if (APSConfig.VANTYPE == (int)EnumVanType.SMATRO) {
                    ...
                    InitKicc(_parkRemainFee, APSConfig.KICCCANCELTIME);
                }
            }
        ...
        private void OnKiccPaymentCompleted(KiccPaymentResult result)
        {
            if (result.IsSuccess) {
                if (result.ResCode == "0000") {
                    ...
                    _cdinfo.ReceiptNum = APSConfig.ReceiptCount.ToString();
        ...
        public void CardCalculateEvent(int msgType, SmartroPacket packet, SmartroApprovalResponse? res)
        {
            ...
            if (msgType == 0 && res != null) {
                ...
                XLogClass?.SaveLogString("CAL", $"승인성공 : {_cdinfo.DealDate} {_cdinfo.DealTime} {_cdinfo.Money}-{_cdinfo.Money}    승인번호={_cdinfo.AcceptNum}");
                _mainForm?.PlaySoundFile($"Comfirm.mp3", AudioRouteState.Idle, 1);
                UiHelpers.ShowPayMsg(this, 0, true);
                _parkRemainFee = 0;
                calOkcheck = true;
        ...
        private void ParkCalForm_Shown(object sender, EventArgs e)
        {
