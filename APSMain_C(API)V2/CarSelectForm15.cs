...
namespace APSMain
{
    public partial class CarSelectForm15 : Form, ISubFormResult<string>, IActiveForm
    {
        ...
        public CarSelectForm15(bool periodtype)
        {
            InitializeComponent();
        ...
        private void CarSelectForm15_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void CarSelectForm15_Load(object sender, EventArgs e)
        {
            ...
            StartAutoCloseTimer(APSConfig.AutoCancleTime2);
        }
        ...
        private void CarSelectForm15_Shown(object sender, EventArgs e)
        {
        ...
        public void ParkCalFormDisplay(string carNum)
        {
            ...
            if (ParkCache.Parkins != null && ParkCache.Parkins.Count > 0) {
                ...
                if (Parkinfo != null) {
                    foreach (var item in ParkCache.Cars) {
                        if (item.Tparkinfo != null) {
                            if (item.Tparkinfo.Carnum.Equals(carNum)) {
                                item.Tparkinfo.Outimage = "";
                                ParkCache.Dispersions.Clear();
                                if (item.Tdisperson != null) {
                                    ParkCache.Dispersions.Add(item.Tdisperson);
                                }
                                ParkCache.Discountinfos.Clear();
                                if (item.Tdiscountinfo != null && item.Tdiscountinfo.Count > 0) {
                                    ParkCache.Discountinfos.AddRange(item.Tdiscountinfo);
                                }
                                ParkCache.Bcardinfos.Clear();
                                if (item.Tbcardinfo != null && item.Tbcardinfo.Count > 0) {
                                    ParkCache.Bcardinfos.AddRange(item.Tbcardinfo);
                                }
                                break;
                            }
                        }
        ...
        private void ShowOrResetCalForm(Tparkinfo? carInfo, Tperiodmember? tmember, int ldmIndex)
        {
            ...
            if (carInfo == null && tmember == null) { return; }
            if (carInfo != null) { _mainForm!._xparkinfo.CopyFrom(carInfo!); }
            ;
